...
function createNumberTexture(num: number) {
  const size = 128;
...
const XConfig = {
  count: 200,
  colors: [0xd82038, 0xffdc4c, 0x6a994e, 0x3a86ff, 0x813d9a],
...
class Z extends Object3D {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    const colorUtils = this.#createColorUtils(config.colors);

    // Create individual spheres with sprites for numbers
    for (let i = 0; i < config.count; i++) {
      ...
      const material = new Y({
        ...
        color: colorUtils.getColorAt(i / config.count),
      });
      ...
      // Create sprite label as child of sphere
      ...
      sprite.position.set(0, 0, 1.01);
      this.labels.push(sprite);
      sphere.add(sprite);
    }

    if (this.light) {
      this.light.color.copy(colorUtils.getColorAt(0));
    }
  }
  ...
  setColors(colors: number[]) {
    // Colors are now baked into materials, so this is less relevant
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      sphere.position.fromArray(this.physics.positionData, 3 * idx);

      if (idx === 0 && this.config.followCursor === false) {
        sphere.scale.setScalar(0);
      } else {
        sphere.scale.setScalar(this.physics.sizeData[idx]);
      }

      if (idx === 0 && this.light) {
        this.light.position.copy(sphere.position);
      }
    }
  }
}
